...
import com.neps.groovydatamapper.utils.ScriptUtils;
...
public abstract class AbstractScriptExecutor implements LoaderExecutor {
    ...
    private NamespacedBinding create() {
        ...
        Closure<Object> findBy = new Closure<Object>(this) {

            @Override
            public Object call(Object... args) {
                if (args == null || args.length != 4) {
                	return new RuntimeException("Provide arguments to find");
                }
                return invokeFindByClosure( (Map<String, Object>) args[0], (Class<? extends Serializable>) args[1], (Serializable) args[2], (Serializable) args[3]);
            };
        };
        ...
        binding.setVariable("findBy", findBy);
    ...
    private Object invokeFindByClosure(Map<String, Object> data, Class<? extends Serializable> clz, Serializable property, Serializable value) {
    	if (data == null || data.isEmpty() || clz == null || property == null || value == null) {
    		throw new RuntimeException("Provide valid arguments to contine to search");
    	}
    	
    	Object obj = data.values().stream()
    			.filter(Objects::nonNull)
    			.filter(clz::isInstance)
    			.map(ScriptUtils::toMap)
    			.filter(m -> m.containsKey(property))
    			.filter(m -> m.containsValue(value))
    			.findFirst()
    			.map(m -> ScriptUtils.toClass(m, clz))
    			.orElse(null);
    	return obj;
    }
